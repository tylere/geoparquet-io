...
"""Integration tests for A5 auto-resolution partitioning."""
...
class TestA5AutoResolutionIntegration:
    ...
    def test_partition_a5_auto_resolution(self, fields_5070_file, tmp_path):
        """Test A5 partitioning with auto-resolution."""
        output_dir = tmp_path / "a5_auto_output"
    ...
    def test_partition_a5_auto_vs_manual(self, fields_5070_file, tmp_path):
        """Test that auto-resolution produces reasonable results compared to manual."""
        auto_dir = tmp_path / "a5_auto"
        manual_dir = tmp_path / "a5_manual"
    ...
    def test_partition_a5_auto_with_constraints(self, fields_5070_file, tmp_path):
        """Test auto-resolution respects max_partitions constraint."""
        output_dir = tmp_path / "a5_constrained"
        ...
        parquet_files = list(output_dir.glob("*.parquet"))
        # A5 at low resolutions has 6, 24, 96 cells, so we'll get coarse partitioning
        assert len(parquet_files) <= 200, "Should respect max_partitions constraint loosely"

    def test_partition_a5_auto_with_hive(self, fields_5070_file, tmp_path):
        """Test auto-resolution with Hive-style partitioning."""
        output_dir = tmp_path / "a5_hive"
        ...
        for subdir in subdirs:
            assert subdir.name.startswith("a5_cell="), f"Subdir should be Hive-style: {subdir.name}"

    def test_partition_a5_auto_error_when_both_auto_and_resolution(
        self, fields_5070_file, tmp_path
    ):
        """Test that specifying both --auto and --resolution raises error."""
        output_dir = tmp_path / "a5_error"

        with pytest.raises(Exception, match="Cannot specify both"):
            partition_by_a5(
                input_parquet=fields_5070_file,
                output_folder=str(output_dir),
                resolution=15,  # Manual resolution
                auto=True,  # And auto - should conflict
                verbose=False,
            )

    def test_partition_a5_preview_with_auto(self, fields_5070_file):
